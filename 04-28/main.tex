...
\subsection{Processi aleatori stazionari}
Vediamo la definizione di \textbf{processo aleatorio stazionario}.
\begin{definition}{Processo aleatorio stazionario}
...
Questo significa che i due processi $X(t)$ e $X(t + \epsilon)$ hanno le stesse caratteristiche per ogni valore di $\epsilon$ e per ogni ordine $N$, ovvero la distribuzione di probabilità congiunta soddisfa la relazione:
$$
f_X(x_1, ..., x_N; t_1, ..., t_N) = f_X(x_1, ..., x_N; t_1 + \epsilon, ..., t_N + \epsilon), \quad \forall \epsilon, N, \{ t_1 , ..., t_N \}
$$
...
Come per le descrizioni, possiamo definire più \textit{ordini} di stazionareità, sulla base del numero di istanti $x_i$ che campioniamo per volta.

\subsubsection{Stazionarietà del primo ordine}
...
\item La funzione \textit{varianza} $\sigma^2_X(t)$:
$$
\sigma^2_X(t) = E\left\{ (X(t) - \eta_X(t))^2 \right\} = \int (x - \eta_X(t)) f_X(x;t) \, dx = P_X(t) - \eta^2_X(t) = P_X - \eta^2_X = \sigma^2_X
$$
\end{itemize}

\subsubsection{Stazionarietà del secondo ordine}
...
Questo implica che $f_X(x_1, x_2; t_1, t_2)$ è dipendente solo dalla differenza temporale $\tau$ fra i 2 istanti considerati:
$$
f_X(x_1, x_2; t_1, t_2) = f_X(x_1, x_2; 0, t_2 - t_1) = f_X(x_1, x_2, \tau)
$$
Questo si ricava direttamente dal fatto che si può porre $\epsilon = -t_1$, e cioè traslare a destra di $t_1$ finchè l'istante $t_1$ non scompare e l'istante $t_2$ diventa costante.

Di conseguenza, Le funzioni di autocorrelazione e autocovarianza di un processo stazionario (almeno) di ordine 2 sono funzione di $\tau$:
\begin{itemize}
\item La funzione \textit{autocorrelazione}:
$$
R_X(t_1, t_2) = E\left[ X(t_1) X(t_1 + \tau) \right] = \int_{x_1 = -\infty}^{\infty} \int_{x_2 = -\infty}^{\infty} x_1 x_2 f_X(x_1, x_2; \tau) \, dx_1 dx_2 = R_X(\tau)
$$
\item La funzione \textit{autocovarianza}:
$$
