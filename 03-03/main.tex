...
Ciò che si voleva capire era la \textbf{capacità} di bit al secondo che si potvano trasferire per un canale di comunicazione digitale (basato su bit 1/0) ad una certa frequenza (in $\mathrm{Hz}$).
...
\end{center}
		...
		In particolare, possiamo dire che il 4G \textit{LTE} sfrutta un apporccio \textit{ibrido} FDNA-TDMA, dove si divide il mezzo sia in slot temporali che bande di frequenza, ottenendo quindi una sorta di \textit{matrice} di slot disponibili per la trasmissione. Questi slot possono quindi essere allocati dalle \textit{base station} ai dispositivi cellulari in maniera dinamica, per permettere la comunicazione sulla rete. 
