...
\item \begin{theorem}{Opposta della trasformata di Fourier}
Vale che:
$$
...
$$
= \int_{\infty}^{-\infty} x(t') e^{j 2 \pi f t'} (-dt') = \int_{-\infty}^{\infty} x(t') e^{-j 2 \pi (-f) t'} \, dt' = X(-f)
$$
che è la tesi. \hfill $\square$
...
\int_{\infty}^{-\infty} x(u) e^{-i 2 \pi f \frac{u}{k}} \, \frac{du}{k} = - \int_{-\infty}^{\infty} x(u) e^{-i 2 \pi f \frac{u}{k}} \, \frac{du}{k} = \frac{1}{-k} X\left(\frac{f}{k}\right) \\
\end{cases}
$$
o in breve:
$$
...
\begin{enumerate}
	\item
Applicando questo teorema, ad esempio, alla funzione rettangolare (vista sempre in 6.1.1), otteniamo:
$$
...
$$
In questo caso possiamo stare tranquilli che le ipotesi di applicabilità dell'equazione di analisi (viste in 6.1) sono soddisfatte, in quanto la funzione $\mathrm{sinc}$ è sempre integrabile su $\mathbb{R}$.

Possiamo ottenere dei risultati interessanti interrogandoci sulla coerenza dimensionale (in termini fisici) dell'ultima espressione ricavata. Infatti il termine $\tau$, che nel contesto dell'$x(t)$ originale rappresentava un tempo, ora va a moltiplicare un tempo (cosa che fisicamente ha poco significato). Possiamo quindi imporre una cosa del tipo:
$$
\tau = 2 B
$$
dove $B$ sarà una \textit{larghezza di banda}, cioè una frequenza. In questo caso otterremo la forma:
$$
2B \mathrm{sinc} (2Bt) \rightarrow \mathrm{rect} \left( \frac{f}{2B} \right) \implies
\mathrm{sinc} (2Bt) \rightarrow \frac{1}{2B} \mathrm{rect} \left( \frac{f}{2B} \right)
$$

Questa formula è interessante in quanto presenta a destra l'equazione del filtro \textit{passa-basso} ideale, detto anche \textit{brickwall filter}. Questo perché, come sappiamo, lo spettro di frequenze della trasformata di Fourier è simmetrico rispetto allo 0, e il grafico della funzione $\mathrm{rect}$ rappresenta una regione a $1$ fino ad una data frequenza ($B$) in entrambe le regioni (negativa e positiva) simmetriche dello spettro. L'$\frac{1}{2B}$ a moltiplicare fa da termine di normalizzazione.

Sulla sinistra abbiamo invece l'antitrasformata, che anticipiamo rappresenterà la risposta all'impulso del filtro sul dominio dello spettro, del filtro brickwall. Questa è una funzione $\mathrm{sinc}$ (per nostra convenzione normalizzata) a frequenza $2B$. Vediamo che nel caso di segnali campionati, la funzione $\mathrm{sinc}$ si comporta da base come la funzione $\sin$ si comporta da base su tutto il dominio $t$.

Il risultato è quindi che un segnale con spettro limitato alla frequenza massima $B$ contiene al massimo informazione a frequenza $2B$ (ricostruibile a partire di funzioni $\mathrm{sinc}$ in base a frequenza $2B$), ovvero se la frequenza $f$ delle sue componenti sarà:
$$
|f| \leq B
$$
...
$$
x(t) = \sum x(n T_s) \mathrm{sinc} \left( f_s (t - n T_s) \right)
$$
per il segnale campionato $x(t)$ sui campioni $n$. $T_s$ sarà il \textit{periodo di campionamento} e $f_s$ la \textit{frequenza di campionamento}, con:
$$
f_s = \frac{1}{T_s}
$$
...
f_s = 2B
$$
cioè campionando alla frequenza di \textit{Nyquist}. Campionare a frequenze ulteriori otterremo sempre lo stesso segnale (anche se in maniera più stabile, in un regime detto di \textit{oversampling}). Ciò che abbiamo ottenuto è quindi un'affermazione del teorema di \textit{campionamento di Nyquist-Shannon}, già introdotto in 3.1.2.

	\item
Applicandolo alla funzione esponenziale monolatera si ha un caso più complesso:
$$
x(t) = e^{-\frac{t}{\tau}} u(t) \rightarrow \frac{\tau}{1 + i 2 \pi f \tau} \implies \frac{\tau}{1 + i 2 \pi t \tau} \rightarrow e^{\frac{f}{\tau}} u(-f), \quad \tau > 0
$$
...
Notiamo che una ulteriore particolarità di questo ultimo spettro è che non è simmetrico: questo è reso possibile dal fatto che il segnale considerato è \textit{complesso} $\in \mathbb{C}$ (quindi non ha ipotesi di simmetria).
...
$$
z(t) = \mathrm{Re} \left\{ \frac{1}{1 + i 2 \pi \frac{t}{\tau} } \right\} = \{ x(t) \}, \quad x(t) = \frac{1}{1 + i 2 \pi \frac{t}{\tau}}
$$

Prendiamo prima la $x(t)$ e discutiamo poi dell'operatore parte reale.
In questo caso ci tornerà utile sfruttare la dualità (teorema 7.4) dell'esponenziale monolatera, cioè dire:
$$
\frac{\tau}{1 + i 2 \pi t \tau} \rightarrow e^{\frac{f}{\tau}} u(-f)  , \quad \tau > 0
$$
che operando il cambio di variabile $\tau' = \frac{1}{\tau}$ diventa:
$$
\frac{1}{\tau' \left( 1 + i 2 \pi \frac{t}{\tau'} \right) } \rightarrow e^{\tau' f} u(-f), \quad
\frac{1}{ 1 + i 2 \pi \frac{t}{\tau'} } \rightarrow \tau' e^{\tau' f} u(-f), \quad
$$
correggendo il fattore $\tau'$ a denominatore (sfruttando la linearità, teorema 7.1). Da qui in poi, chiamiamo $\tau'$ semplicemente $\tau$ e proseguiamo.
...
$$
\mathrm{Re} \{ x(t) \} = \frac{x(t) + x^*(t)}{2}
$$
e calcoliamo $x^*(t)$ come:
$$
x^*(t) = \left( \frac{1}{1 + i 2 \pi \frac{t}{\tau} }\right)^* = \left( \frac{1 - i 2 \pi \frac{t}{\tau} }{ \left( 1 + i 2 \pi \frac{t}{\tau} \right) \left( 1 - i 2 \pi \frac{t}{\tau} \right) } \right)^*
$$
$$
= \frac{1 + 2 \pi \frac{t}{\tau} }{ \left( 1 + i 2 \pi \frac{t}{\tau} \right) \left( 1 - i 2 \pi \frac{t}{\tau} \right) } = \frac{1}{ 1 - i 2 \pi \frac{t}{\tau} }
$$
...
$$
\frac{x(t) + x^*(t)}{2} = \frac{1}{2} \left( e^{\tau f} u(-f) + e^{-\tau f} u(f) \right)
$$
...
\frac{1}{ 1 - i 2 \pi \frac{t}{\tau'} } \rightarrow \tau' e^{-\tau' f} u(f), \quad
$$
usando sempre la linearità. Ricordiamo che dobbiamo prendere la versione con $f$ positiva della trasformata notevole, in quanto la sostituzione che facciamo prevede un $\tau'$ negativo (che altrimenti divergerebbe), come visto in 6.1.1. Il risultato, in ogni caso, è lo stesso.
