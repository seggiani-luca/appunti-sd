...
$$
dove l'$(x - \tau)$ indica effettivamente lo scostamento temporale dato dalla distanza $2d$ complessiva che il segnale deve percorrere. Prendiamo la radice quadrata dell'attenuazione $\beta$ in quanto i segnali $x(t)$ e $y(t)$ si trovano nel dominio di ampiezza, e non potenza, per cui dobbiamo passare alle radici.
...
\begin{itemize}
	\item $24.0 \, \mathrm{MHz}$ - $25.25 \, \mathrm{GHz}$ per il \textbf{SRR} (\textit{Short Range Radar});
	\item $76 \, \mathrm{MHz}$ - $81 \, \mathrm{GHz}$ per il \textbf{MRR} (\textit{Medium Range Radar}).
\end{itemize}

Notiamo anche che maggiore frequenza significa minore lunghezza d'onda, in quanto in generale:
$
\lambda = \frac{c}{f_0}
$
Questo significherà che la realizazzione dei radar è un processo che richiede l'ottimizzazione della frequenza di modulazione, dove bisogna bilanciare la risoluzione del radar con l'attenuazione incontrata.
...
\mathrm{tri}(t) =
\begin{cases}
	1 - |2\frac{t}{\tau}|, \quad |t| \leq \frac{\tau}{2} \\
	0, \quad |t| > \frac{\tau}{2}
\end{cases}
$$
