...
ammesso che:
\begin{itemize}
	\item $x(t)$ sia continua $C^1$ a pezzi su $\mathbb{R}$;
	\item $x(t)$ sia integrabile su tutto $\mathbb{R}$.
\end{itemize}
\end{definition}
...
\begin{center}
	\includegraphics[scale=0.6]{../figures/fourier_sinesum.png}
\end{center}
...
\begin{center}
	\includegraphics[scale=0.6]{../figures/fourier_voxhumana.png}
\end{center}
...
$$
X(f) = \int_{-\infty}^{\infty} x(t) e^{-i 2 \pi f t} \, dt = \int_0^{\infty} e^{-\frac{t}{\tau}} e^{-i 2 \pi f t} \, dt = \int_0^\infty e^{-t\left( \frac{1}{\tau} + i 2 \pi f \right)} \, dt 
$$

$$
= -\frac{e^{-t\left( \frac{1}{\tau} + i 2 \pi f \right)}}{\frac{1}{\tau} + i 2 \pi f} \Big|^{\infty}_0
...
$$
X(f) = \int_{-\infty}^{\infty} x(t) e^{-i 2 \pi f t} \, dt = \int_{-\infty}^0 e^{-\frac{t}{\tau}} e^{-i 2 \pi f t} \, dt = \int_{-\infty}^0 e^{-t\left( \frac{1}{\tau} + i 2 \pi f \right)} \, dt 
$$
$$
= -\frac{e^{-t\left( \frac{1}{\tau} + i 2 \pi f \right)}}{\frac{1}{\tau} + i 2 \pi f} \Big|^{-\infty}_0
= -\frac{\tau}{1 + i 2 \pi f \tau}
...
\begin{center}
	\includegraphics[scale=0.6]{../figures/fourier_exp.png}
\end{center}

Abbiamo quindi trovato una trasformata \textit{fondamentale} (effettivamente una coppia di esse):
$$
x(t) = e^{-\frac{t}{\tau}} u(t) \Leftrightarrow X(f) = \frac{\tau}{1 + i 2 \pi f \tau}, \quad \tau > 0
$$
$$
x(t) = e^{-\frac{t}{\tau}} u(-t) \Leftrightarrow X(f) = -\frac{\tau}{1 + i 2 \pi f \tau}, \quad \tau < 0
$$
...
= - \frac{1}{i 2 \pi f} e ^{-i 2 \pi f t} \Big|^{\frac{\tau}{2}}_{-\frac{\tau}{2}} 
$$
$$
= - \frac{1}{i 2 \pi f} \left( e^{-i \pi f \tau} - e^{i 2 \pi f \tau} \right)
= - \frac{1}{i 2 \pi f} \cdot - 2 i \sin (\pi f t) = \frac{\sin(\pi f \tau)}{\pi f} 
$$
\hfill $\square$

Notiamo che in questo caso non dobbiamo preoccuparci rispetto alle condizioni in 6.1, in quanto queste sono automaticamente rispettate, ovvero $x(t)$ è integrabile su $\mathbb{R}$ per qualsiasi valore di $\tau$ (finché $\tau$ è finito).

\newpage

Vediamo quindi la funzione ottenuta su un grafico, generato con lo script precedente:
\begin{center}
	\includegraphics[scale=0.6]{../figures/fourier_step.png}
\end{center}
...
Abbiamo quindi trovato un'altra trasformata \textit{fondamentale}:
$$
