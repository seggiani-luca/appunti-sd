...
$$
già incontrata in fondamenti di automatica. Sempre da fondamenti di automatica, ricordiamo che le proprietà importanti della delta di Dirac sono:
\begin{enumerate}
...
\noindent
\textbf{\textsf{Segnale con ritardo}}
...
$$
da cui segue direttamente (per il teorema della dualità):
$$
...
\noindent
\textbf{\textsf{Segnali periodici}}

\noindent
L'ultima proprietà trovata può tornarci utile per calcolare la trasformata di funzioni periodiche. Abbiamo detto che questo non era possibile in quanto è necessario che i segnali che trasformiamo (con la trasformata non generalizzata) abbiano energia finita.
...
\frac{\delta(f - f_0) . \delta(f + f_0)}{2j} = X_c(f)
$$
