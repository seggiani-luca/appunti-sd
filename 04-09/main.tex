...
Questo teorema vale anche per intervalli disgiunti (nel caso di funzioni continue non iniettiva), in quanto se in generale $g(X)$ è una funzione continua, che non è costante in alcun intervallo (altrimenti anche se $X$ fosse continua non lo sarebbe $Y$), si ha:
$$
...
y = g(x)
$$
con $y$ fisso (nel punto in cui vogliamo calcolare $f_Y (y)$). Chiaramente il numero $N$ di $x_i$ varia al variare di $y$ (per una varietù di funzioni non inettive). Inoltre, se per un certo $y^*$ l’equazione $y = g(x)$ non ha soluzione, risulterà:
$$
f_Y (y^*) = 0
$$
