...
\subsubsection{Codici a blocco lineari}
I codici a blocco \textbf{lineari} sono rappresentati da trasformazioni lineari di stringhe di $k$ bit, dette \textit{blocchi} $\mathbf{b}$:
$$
\mathbf{b} = [ b_1, ..., b_k ]
$$
notiamo che definiamo questi blocchi come vettori \textbf{riga} (!).
...
$$
dove $G$ è una matrice $k \times n$ detta \textit{matrice generatrice} del codice. Le $\mathbf{g}_i$ a questo punto sono le $k$ righe di dimensione $n$ della matrice. Notiamo che anche $\mathbf{c}$ è un vettore \textit{riga} (!!).
